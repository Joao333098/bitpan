...
{elements_text}
        ...
        def _prune_old_messages(self, keep_last: int = 6) -> None:
                """Remove images from state messages older than keep_last steps to reduce context size"""
                state_messages = [msg for msg in self._messages if msg.is_state_message]
                to_strip = state_messages[:-keep_last] if len(state_messages) > keep_last else []
                for msg in to_strip:
                        msg.content = [c for c in msg.content if not hasattr(c, 'image_b64')]
...
{model_output.model_dump_json(indent=2, include={"thought", "action", "summary"}).strip()}
                ...
                self._prune_old_messages(keep_last=6)
