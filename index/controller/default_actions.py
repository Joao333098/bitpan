...
def register_default_actions(controller):
    ...
    @controller.action()
    async def execute_javascript(code: str, browser: Browser):
        """Execute JavaScript code in the current browser page (like the DevTools console).
        Use this to manipulate the page, skip videos, bypass restrictions, extract data, etc.
        The result of the last expression is returned.

        Examples:
          - Skip a video: document.querySelector('video').currentTime = document.querySelector('video').duration
          - Click hidden button: document.querySelector('#skip-btn').click()
          - Get page title: document.title
          - Remove overlay: document.querySelector('.overlay').remove()
        ...
        try:
            ...
            msg = f"JavaScript executed successfully. Result: {result_str}"
            logger.info(msg)
            return ActionResult(content=msg)
        except Exception as e:
            return ActionResult(error=f"JavaScript error: {str(e)}")

    @controller.action()
    async def read_console_logs(last_n: int, browser: Browser):
        """Read the browser's console log messages (errors, warnings, logs).
        Use this to debug page issues, check for errors, or inspect what scripts are doing.

        Args:
            last_n: Number of most recent console messages to return (e.g. 50).
        """
        logs = browser._console_logs
        if not logs:
            return ActionResult(content="Console is empty — no messages captured yet.")
        recent = logs[-last_n:]
        result = f"Console logs (last {len(recent)} entries):\n" + "\n".join(recent)
        return ActionResult(content=result)

    @controller.action()
    async def clear_console_logs(browser: Browser):
        """Clear all captured browser console logs."""
        browser._console_logs.clear()
        return ActionResult(content="Console logs cleared.")

    @controller.action()
    async def web_search(query: str):
        """Search the internet for current information about any topic.
        Use this when you need to find up-to-date information, facts, news, or anything that requires searching the web.
        ...
        try:
            async with httpx.AsyncClient(timeout=12, follow_redirects=True) as client:
                ...
                if parts:
                    ...
                    return ActionResult(content=result_text)

                return ActionResult(content=f"No results found for '{query}'. Try searching with a browser.")
        except Exception as e:
            logger.warning(f"Web search failed: {e}")
            return ActionResult(error=f"Web search failed: {e}")
    ...
    async def click_element(index: int, browser: Browser, wait_after_click: bool = False):
        ...
        if state is None:
            return ActionResult(error="Browser state is not ready yet. Please wait and retry.")
